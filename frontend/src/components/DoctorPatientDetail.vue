  ...
  <div class="detail-container">
    ...
    <div class="flex items-center justify-between mb-8">
      <div class="flex items-center gap-4">
        <button
          @click="$emit('back')"
          class="p-2 hover:bg-slate-100 rounded-xl transition-colors text-slate-500 hover:text-indigo-600"
        >
          <ArrowLeft :size="24" />
        </button>
        <div>
          <h2 class="text-2xl font-black text-slate-900">
            {{ patient?.full_name || 'Chi tiết bệnh nhân' }}
          </h2>
          <div class="flex items-center gap-3 mt-1">
            <span
              class="px-2.5 py-0.5 rounded-full bg-emerald-100 text-emerald-700 text-xs font-bold uppercase tracking-wider"
            >
      ...
      <div class="flex gap-3">
        <button
          @click="currentTab = 'medical_record'"
          class="px-4 py-2 bg-white border border-slate-200 text-slate-600 rounded-xl font-bold text-sm hover:bg-slate-50 transition-colors flex items-center gap-2"
        >
          <FileText :size="16" /> Hồ sơ bệnh án
        </button>
        <button
          @click="navigateToMessages"
          class="px-4 py-2 bg-indigo-600 text-white rounded-xl font-bold text-sm hover:bg-indigo-700 shadow-lg shadow-indigo-500/20 transition-colors flex items-center gap-2"
        >
      ...
      <div v-else-if="currentTab === 'medical_record'" class="space-y-6">
        <div class="flex justify-between items-center">
          ...
          <button
            ...
            class="px-4 py-2 bg-indigo-50 text-indigo-600 rounded-lg font-bold text-sm hover:bg-indigo-100"
          >
            Lưu thay đổi
          </button>
        </div>
        ...
        <div class="grid grid-cols-2 gap-6">
          <div class="space-y-4">
            <div>
              ...
              <textarea
                ...
                rows="3"
                class="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium focus:ring-2 focus:ring-indigo-500/20 outline-none"
              ></textarea>
            </div>
            <div>
              <label class="block text-sm font-bold text-slate-500 mb-1">Triệu chứng</label>
              <textarea
                v-model="medicalRecord.symptoms"
                rows="3"
                class="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium focus:ring-2 focus:ring-indigo-500/20 outline-none"
              ></textarea>
            </div>
          ...
          <div class="space-y-4">
            <div>
              ...
              <textarea
                v-model="medicalRecord.treatment_plan"
                rows="3"
                class="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium focus:ring-2 focus:ring-indigo-500/20 outline-none"
              ></textarea>
            </div>
            <div class="grid grid-cols-3 gap-4">
              <div>
                ...
                <input
                  v-model="medicalRecord.height_cm"
                  type="number"
                  class="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium outline-none"
                />
              </div>
              <div>
                <label class="block text-sm font-bold text-slate-500 mb-1">Cân nặng (kg)</label>
                <input
                  v-model="medicalRecord.weight_kg"
                  type="number"
                  class="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium outline-none"
                />
              </div>
              <div>
                ...
                <select
                  v-model="medicalRecord.blood_type"
                  class="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium outline-none"
                >
              ...
              >
                <td class="py-3 px-4 text-sm font-medium text-slate-900">
                  ...
                  <span class="text-xs text-slate-500"
                    >{{ formatTime(log.start_time) }} - {{ formatTime(log.end_time) }}</span
                  >
                </td>
      ...
      <div v-else-if="currentTab === 'notes'">
        ...
        <div v-if="showAddNote" class="mb-6 p-4 bg-slate-50 rounded-xl border border-slate-200">
          <input
            ...
            placeholder="Tiêu đề"
            class="w-full mb-3 p-2 rounded-lg border border-slate-200 text-sm font-bold outline-none"
          />
          <textarea
            v-model="newNote.content"
            placeholder="Nội dung ghi chú..."
            rows="3"
            class="w-full mb-3 p-2 rounded-lg border border-slate-200 text-sm outline-none"
          ></textarea>
          <div class="flex justify-end gap-2">
            <button
              @click="showAddNote = false"
              class="px-3 py-1.5 text-slate-500 text-sm font-bold hover:bg-slate-200 rounded-lg"
            >
              Hủy
            </button>
            <button
              @click="saveNote"
              class="px-3 py-1.5 bg-indigo-600 text-white text-sm font-bold rounded-lg hover:bg-indigo-700"
            >
              Lưu
            </button>
          </div>
        ...
        <div class="space-y-4">
          <div
            v-for="note in notes"
            :key="note.note_id"
            class="p-4 border border-slate-100 rounded-xl hover:shadow-md transition-shadow"
          >
            ...
            <div class="mt-2 text-xs font-bold text-indigo-600">
              Bác sĩ: {{ note.doctor_name || 'Unknown' }}
            </div>
          </div>
          <div v-if="notes.length === 0" class="text-center py-8 text-slate-500">
            Chưa có ghi chú nào
          </div>
        </div>
      </div>
