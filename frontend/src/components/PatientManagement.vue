<template>
  <div v-if="!selectedPatient" class="h-full flex flex-col space-y-6">
    <!-- Header Section -->
    <div
      class="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white p-6 rounded-3xl border border-slate-100 shadow-sm"
    >
      <div>
    ...
    <div class="flex flex-col md:flex-row gap-4">
      ...
      <div class="flex gap-2">
        <select
          ...
          class="px-4 py-3 bg-white border border-slate-200 rounded-xl font-medium text-slate-600 outline-none focus:border-indigo-500"
        >
    ...
    <!-- Patient List -->
    <div
      class="flex-1 bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden flex flex-col"
    >
      <div class="overflow-x-auto custom-scrollbar flex-1">
        <table class="w-full text-left border-collapse">
          <thead>
            <tr class="bg-slate-50 border-b border-slate-100">
              <th class="px-6 py-4 text-xs font-black text-slate-500 uppercase tracking-wider">
                Bệnh Nhân
              </th>
              <th class="px-6 py-4 text-xs font-black text-slate-500 uppercase tracking-wider">
                Trạng Thái
            ...
            >
              ...
              <td class="px-6 py-4">
                <div class="flex items-center gap-2 text-sm font-medium text-slate-700">
                  ...
                  {{ patient.progress || 0 }}% hoàn thành
                </div>
              </td>
              <td class="px-6 py-4">
                <div class="flex items-center gap-2 text-sm font-medium text-slate-700">
                  <Activity :size="16" class="text-slate-400" />
                  {{ getActivityFrequency(patient.status) }}
                </div>
              </td>
              <td class="px-6 py-4">
                <div class="flex items-center gap-2 text-sm font-medium text-slate-700">
    ...
    >
      <div
        class="bg-white rounded-3xl p-8 w-full max-w-lg shadow-2xl transform scale-100 transition-all border border-slate-100"
      >
        ...
        <form @submit.prevent="addNewPatient" class="space-y-5">
          ...
          <div class="flex justify-end gap-3 pt-4">
            ...
            <button
              type="submit"
              class="px-6 py-3 bg-indigo-600 text-white rounded-xl text-sm font-bold hover:bg-indigo-700 shadow-lg shadow-indigo-500/30 transition-all"
            >
              Thêm Mới
            </button>
          </div>
...
const filteredPatients = computed(() => {
  return patients.value.filter((p) => {
    ...
    if (filterStatus.value !== 'All') {
      const statusMap = { Stable: 'active', Review: 'needs_attention', Critical: 'inactive' }
      if (p.status !== statusMap[filterStatus.value]) return false
    }
...
function getStatusClass(status) {
  ...
  if (status === 'needs_attention') return 'bg-amber-50 text-amber-600 border-amber-200'
  return 'bg-slate-50 text-slate-600 border-slate-200'
}

function getStatusLabel(status) {
  if (status === 'active') return 'Hoạt động'
  if (status === 'needs_attention') return 'Cần theo dõi'
...
function getActivityFrequency(status) {
  if (status === 'active') return 'Thường xuyên'
  if (status === 'needs_attention') return 'Thỉnh thoảng'
  return 'Ít hoạt động'
}
...
async function loadPatients() {
  try {
    ...
    // Use patients-with-status endpoint for real status data
...
async function deletePatient(id) {
  if (
    !confirm('Bạn có chắc muốn xóa bệnh nhân này? Hành động này sẽ xóa tất cả dữ liệu liên quan.')
  )
    return
