<template>
  <div class="flex h-[calc(100vh-14rem)] gap-6 bg-slate-50/50">
    <!-- Sidebar: Doctor List -->
    <div
          ...
          <div class="flex-1 text-left">
            <h4
              :class="[
                ...
                selectedDoctor?.user_id === doctor.user_id ? 'text-indigo-900' : 'text-slate-700',
              ]"
            >
              {{ doctor.full_name }}
            </h4>
    ...
    >
      ...
      >
        ...
        <h3 class="text-xl font-bold text-slate-600">Chọn bác sĩ để bắt đầu chat</h3>
      </div>
      ...
      <template v-else>
        ...
        >
          <div class="flex items-center gap-4">
            ...
            <div>
              <h3 class="text-lg font-black text-slate-900">{{ selectedDoctor.full_name }}</h3>
        ...
        >
          ...
          >
            ...
            >
              {{ msg.sender_id === currentUserId ? 'Me' : getInitials(selectedDoctor.full_name) }}
            </div>
...
const filteredDoctors = computed(() => {
  return doctors.value.filter((d) => {
    const name = removeVietnameseTones((d.full_name || '').toLowerCase())
...
async function loadMessages() {
  ...
  try {
    ...
    const res = await fetch(
      `${API_BASE}/messages/${currentUserId.value}/${selectedDoctor.value.user_id}`,
      {
        headers: { Authorization: `Bearer ${token}` },
      },
    )
    if (res.ok) {
      ...
      // API returns pagination: { items: [...], ... } or just list
...
async function sendMessage() {
  ...
  try {
    const payload = {
      receiver_id: selectedDoctor.value.user_id,
