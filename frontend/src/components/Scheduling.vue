<template>
  <div class="h-full flex flex-col space-y-6" data-isolated-ui>
    ...
    >
      ...
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
        ...
        <div class="flex items-center gap-3">
          ...
          <button
            @click="openNew = true"
            class="px-5 py-2.5 bg-white text-indigo-600 rounded-xl text-sm font-bold hover:bg-indigo-50 transition-all shadow-lg flex items-center gap-2"
          >
            <Plus :size="18" />
    ...
    <div class="flex h-full gap-6 overflow-hidden">
      <!-- Calendar Section (Flexible width) -->
        ...
        >
          ...
          >
            <div class="flex items-center gap-4">
              <button
                @click="today"
                class="px-4 py-2 border-2 border-indigo-200 rounded-xl text-sm font-bold text-indigo-600 hover:bg-indigo-50 transition-colors shadow-sm"
              >
                Hôm nay
              </button>
              <div class="flex items-center gap-2">
                <button
                  @click="prev"
                  class="p-2 hover:bg-indigo-50 rounded-full text-indigo-500 hover:text-indigo-700 transition-colors"
                >
                  <ChevronLeft :size="20" />
                </button>
                <button
            ...
            <div class="hidden lg:flex items-center gap-4 text-xs font-bold text-slate-600">
              <div
                class="flex items-center gap-2 px-3 py-1.5 bg-blue-50 rounded-lg border border-blue-100"
              >
                <span class="w-3 h-3 rounded-full bg-blue-500"></span> Khám bệnh
              </div>
              <div
                class="flex items-center gap-2 px-3 py-1.5 bg-emerald-50 rounded-lg border border-emerald-100"
              >
                <span class="w-3 h-3 rounded-full bg-emerald-500"></span> Tái khám
              </div>
              <div
                class="flex items-center gap-2 px-3 py-1.5 bg-amber-50 rounded-lg border border-amber-100"
              >
          ...
          <div v-if="currentView === 'Tháng'" class="flex-1 flex flex-col min-h-0 w-full">
            ...
            <!-- Days Grid -->
            <div class="flex-1 grid grid-cols-7 w-full overflow-y-auto custom-scrollbar">
              <div
              ...
              >
                ...
                <div class="flex-1 overflow-y-auto custom-scrollbar space-y-1.5">
                  ...
                  <div
                    ...
                    class="text-xs font-bold text-indigo-600 px-2 py-1"
                  >
                  ...
                  >
                    ...
                    >
                      <div class="flex items-center justify-between mb-2">
                        ...
                        <span
                          class="text-sm font-bold opacity-75 px-2 py-1 rounded-md bg-white/50"
                          >{{ ev.type }}</span
                        >
                      </div>
                    ...
                    <div
                      v-if="getEventsForTimeSlot(current, hour - 1).length === 0"
    ...
    >
      <div
        class="bg-white rounded-3xl p-8 w-full max-w-lg shadow-2xl border border-slate-100 transform scale-100 transition-all"
      >
        <div class="flex items-center justify-between mb-6">
          <h3 class="text-2xl font-black text-slate-900">
            {{ editingEvent ? 'Sửa Cuộc Hẹn' : 'Cuộc Hẹn Mới' }}
          </h3>
          <button
            @click="closeModal"
            class="p-2 hover:bg-slate-50 rounded-xl transition-colors text-slate-400 hover:text-slate-900"
          >
            <X :size="20" />
          </button>
        </div>
        ...
        <form @submit.prevent="saveEvent" class="space-y-5">
          <div>
            ...
            <select
              ...
              required
              class="w-full px-4 py-3 bg-slate-50 border-2 border-slate-200 rounded-xl text-sm focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none transition-all font-medium"
            >
          ...
          <div class="flex justify-between items-center pt-4">
            <button
              ...
              type="button"
              @click="deleteEvent"
              class="px-4 py-3 bg-red-50 text-red-600 rounded-xl text-sm font-bold hover:bg-red-100 transition-colors flex items-center gap-2"
            >
              <Trash2 :size="18" />
              Xóa
            </button>
    ...
    <div
      ...
      class="fixed inset-0 bg-slate-900/70 backdrop-blur-sm flex items-center justify-center z-[110] p-4"
    >
      <div
        class="bg-white rounded-3xl p-8 w-full max-w-md shadow-2xl border border-slate-100 transform scale-100 transition-all"
      >
    ...
    <div
      v-if="showError"
      class="fixed inset-0 bg-slate-900/70 backdrop-blur-sm flex items-center justify-center z-[120] p-4"
    >
      <div
        class="bg-white rounded-3xl p-8 w-full max-w-md shadow-2xl border border-slate-100 transform scale-100 transition-all"
      >
        <div class="flex flex-col items-center text-center">
          ...
          <div class="w-16 h-16 bg-indigo-100 rounded-full flex items-center justify-center mb-4">
            <AlertTriangle :size="32" class="text-indigo-600" />
          </div>

          <!-- Title -->
          <h3 class="text-2xl font-black text-slate-900 mb-2">Thông báo</h3>
          ...
          <div class="w-full">
            <button
              ...
              class="w-full px-6 py-3 bg-indigo-600 text-white rounded-xl text-sm font-bold hover:bg-indigo-700 shadow-lg shadow-indigo-500/30 transition-all"
            >
              Đóng
            </button>
          </div>
...
const typeMap = {
  ...
  Khám: 'consultation',
  'Tái khám': 'followup',
  'Vật lý trị liệu': 'therapy',
  'Đánh giá bài tập': 'assessment',
}
...
function mapToBackend(frontendType) {
  return typeMap[frontendType] || 'consultation'
}
...
async function saveEvent() {
  ...
  const isDuplicate = events.value.some((event) => {
    if (editingEvent.value && event.id === editingEvent.value.id) return false

    const eventDate = new Date(event.date)
    return (
      event.patient_id === form.value.patient_id &&
  ...
  if (isDuplicate) {
    errorMessage.value = 'Lịch hẹn này đã tồn tại! Vui lòng chọn thời gian khác.'
  ...
  if (startDateTime < now) {
    // For new events, strictly block
    if (!editingEvent.value) {
      errorMessage.value = 'Ngày hẹn không hợp lệ!'
      showError.value = true
      return
    }

    // For editing events, block only if time changed to past
    // If just updating notes of a past meeting, allow it
    const oldDate = new Date(editingEvent.value.date)
    const [oldHour, oldMinute] = editingEvent.value.time.split(':')
    oldDate.setHours(oldHour, oldMinute)

    if (oldDate.getTime() !== startDateTime.getTime()) {
      errorMessage.value = 'Không thể dời lịch hẹn về quá khứ!'
      showError.value = true
      return
    }
  }
  ...
  const payload = {
    ...
    session_type: mapToBackend(form.value.type), // Map to backend enum
  ...
  try {
    ...
    if (editingEvent.value) {
      url = `${API_URL}/schedules/${editingEvent.value.id}`
      method = 'PUT'
    }
...
async function loadData() {
  try {
    ...
    if (meRes.ok) {
      ...
      doctorId.value = userData.user_id
    }

    // Load patients for dropdown
    const patRes = await fetch(`${API_URL}/patients-with-status`, {
      headers: { Authorization: `Bearer ${token}` },
    })
    if (patRes.ok) {
    ...
    if (schedRes.ok) {
      ...
      events.value = data.map((s) => ({
        ...
        name: s.patient_name || 'Bệnh nhân',
        date: new Date(s.start_time),
        time: new Date(s.start_time).toLocaleTimeString('vi-VN', {
          hour: '2-digit',
          minute: '2-digit',
        }),
        type: mapToFrontend(s.session_type), // Map from backend enum
        notes: s.notes,
      }))
    }
  } catch (error) {
...
function edit(ev) {
  ...
  form.value = {
    ...
    time: ev.time,
    type: ev.type, // Should already be frontend mapped
    notes: ev.notes,
  }
  openNew.value = true
}
