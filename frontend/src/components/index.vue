<template>
  <div
    class="flex h-screen w-full bg-slate-50 font-sans text-slate-900 overflow-hidden selection:bg-indigo-100 selection:text-indigo-900 relative"
    data-isolated-ui
  >
    ...
    >
      ...
      <div
        :class="[
          'border-b border-slate-100 relative z-10 transition-all',
        ...
        <div :class="['flex items-center', sidebarCollapsed ? 'justify-center' : 'gap-3']">
          <div v-if="!sidebarCollapsed" class="relative group">
            ...
            <img
              ...
              alt="Dr. Minh"
              class="relative w-11 h-11 rounded-full shadow-lg ring-4 ring-white object-cover"
            />
          </div>
          ...
          <div v-if="!sidebarCollapsed" class="flex-1 min-w-0">
            ...
            <p class="text-[10px] text-slate-500 font-bold uppercase tracking-wider">
              {{ currentUser?.role === 'doctor' ? 'Bác sĩ' : 'Bệnh nhân' }}
            </p>
          </div>
          ...
          >
            <component
              ...
              :size="16"
              class="group-hover:scale-110 transition-transform font-bold"
            />
          </button>
        </div>
      </div>
      ...
      <nav class="flex-1 px-3 py-4 space-y-6 overflow-y-auto custom-scrollbar relative z-10">
        <div v-for="(group, groupName) in groupedMenu" :key="groupName">
          <h3
            ...
            class="px-3 mb-2 text-[9px] font-black text-slate-400 uppercase tracking-widest"
          >
          ...
          <div class="space-y-1">
            ...
            >
              ...
              <ChevronRight
                v-if="active === item.id && !sidebarCollapsed"
                :size="14"
                class="ml-auto text-white/80 relative z-10"
              />
            </button>
          </div>
        </div>
    ...
    <main class="flex-1 flex flex-col min-w-0 overflow-hidden relative z-10 bg-slate-50/50">
      <!-- Premium Top Bar -->
      <header
        class="h-16 md:h-20 flex-shrink-0 px-5 md:px-8 flex items-center justify-between bg-white/80 backdrop-blur-xl border-b border-slate-200/60 sticky top-0 z-40"
      >
        <!-- Left: Title -->
        <div class="flex flex-col justify-center flex-1">
          <h2 class="text-xl md:text-2xl font-black text-slate-900 tracking-tight truncate">
            {{ title }}
          </h2>
        ...
        <div class="flex justify-end items-center gap-3">
          <div class="relative group hidden lg:block w-64">
            <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none z-10">
              <Search
                class="text-slate-400 group-focus-within:text-indigo-600 transition-colors"
                :size="16"
              />
            </div>
            <input
          ...
          <div class="relative">
            <button
              ...
              class="p-2 text-slate-500 hover:text-indigo-600 hover:bg-indigo-50 rounded-xl transition-all relative group flex items-center justify-center"
            >
              <Bell :size="20" class="relative" />
              <span
                v-if="unreadCount > 0"
                class="absolute top-1 right-1 w-5 h-5 bg-red-500 rounded-full border-2 border-white text-[10px] text-white font-bold flex items-center justify-center"
              >
                    ...
                    <div class="flex items-start gap-3">
                      <div
                        :class="[
                          ...
                          !notif.is_read ? 'bg-indigo-500' : 'bg-slate-300',
                        ]"
                      ></div>
                      <div class="flex-1 min-w-0">
                        <p class="font-bold text-sm text-slate-900">{{ notif.title }}</p>
